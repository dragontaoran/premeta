...
bool VcfUtils::IsSegmentDone(const ProcessLineResponse& response_type,
  ...
  if (output_options.segment_type_ != SegmentType::GENE) {
    ...
    cout << "ERROR: Unexpected Segment Type: "
...
bool VcfUtils::ParseNucleotide(const string& input, Nucleotide* n) {
  if (input.length() != 1) return false;
  *n = GetNucleotide(input.at(0));
  return *n != Nucleotide::NUCLEOTIDE_NONE;
}
...
bool VcfUtils::AddVariantToFilter(const string& line, GroupingFilter* filter) {
  ...
  if (line.find(":") == string::npos) {
    ...
    int64_t position;
    if (!Stoi(line.substr(delimeter_pos + 1), &position)) {
...
bool VcfUtils::IsInfoFiltered(
  ...
  for (const string& field : fields) {
    ...
    if (key == "AF") {
      ...
      Split(value, ",", &frequencies);
      Stod(frequencies[0], minor_freq);
      if (frequencies.size() > 1) {
        Stod(frequencies[1], tri_freq);
      }
    }
  }
  return false;
...
void VcfUtils::SetVariantsToFilterFromFile(const string& filename,
  ...
  while (getline(file, line)) {
    ...
    if ((line_parts.size() == 2)) {
      ...
      if (!Stoi(line_parts[1], &range_start)) {
        cout << "ERROR in parsing file " << filename
             << ": Unable to parse line " << line_index
      ...
      if (!Stoi(line_parts[2], &range_end)) {
        cout << "ERROR in parsing file " << filename
             << ": Unable to parse line " << line_index
...
void VcfUtils::GetGenePositionMapFromFile(
  ...
  while (getline(file, line)) {
    ...
    // Parse range start and end.
    int64_t range_start, range_end;
    if (!Stoi(line_parts[2], &range_start)) {
      cout << "ERROR in parsing file " << filename
           << ": Unable to parse line " << line_index
           << " which has unparsable range start:\n"
           << line << endl;
    ...
    if (!Stoi(line_parts[3], &range_end)) {
      cout << "ERROR in parsing file " << filename
           << ": Unable to parse line " << line_index
           << " which has unparsable range end:\n"
           << line << endl;
...
bool VcfUtils::ProcessHeaderLine(
  ...
  if (!HasPrefixString(line, "#CHROM")) return false;
  vector<string> temp_header;
  if (!Split(line, "\t", &temp_header)) return false;

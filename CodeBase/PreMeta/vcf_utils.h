...
class VcfUtils {
 public:
  ...
  void ProcessVcfFile(const string& filename,
  ...
  void ProcessVcfFile(VcfGroup* vcf_group,
                      int64_t* next_char_to_read_pos,
                      int* num_lines_processed,
                      FilteringCounts* filtering_counts,
                      vector<vector<SnpType>>* snp_values_per_sample,
  ...
  void ProcessVcfFile(const vector<VcfGroup*>& vcf_groups,
                      int64_t* next_char_to_read_pos,
                      int* num_lines_processed,
                      FilteringCounts* filtering_counts,
                      vector<vector<SnpType>>* snp_values_per_sample,
  ...
  void GetGenePositionMapFromFile(
  ...
  void SetSamplesToFilterFromFile(const string& filename,
  ...
  // is determined by calling function.
  void SetVariantsToFilterFromFile(const string& filename,
 ...
 private:
  ...
  //     appropriately.
  ProcessLineResponse ProcessLine(
      const string& line,
  ...
  bool IsSegmentDone(const ProcessLineResponse& response_type,
  ...
  bool AddVariantToFilter(const string& line, GroupingFilter* filter);
  ...
  bool GetVariantsToKeep(const OutputOptions& output_options,
                         GroupingFilter* filter, int* num_variants_to_keep);

  // Parses SNP position from POSITION column of the vcf file.
  bool GetPosition(const string& column, int64_t* position);

  // Returns the Nucleotide type that c represents.
  Nucleotide GetNucleotide(const char c);
